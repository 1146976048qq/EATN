...
class EATN(BertForSequenceClassification):
    """
    ...
    def forward(self, source_input_ids, source_token_type_ids=None, source_attention_mask=None, source_domain_labels=None, source_aspect_labels=None,
               target_input_ids=None, target_token_type_ids=None, target_attention_mask=None, target_domain_labels=None, target_aspect_labels=None,
               is_training=False):
        """
        Args:
          input_ids: (batch, seq_len), word index of text, start with [CLS] and end with [SEP] token ids
        ...
        # aspect classifier
        source_aspect_logits = self.aspect_classifier(source_output)
        ...
        if is_training:
          ...
          source_aspect_prediction = F.softmax(source_aspect_logits, dim=-1)
